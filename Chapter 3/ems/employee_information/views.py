...
from employee_information.forms import EmployeeForm
...
@login_required
def checking_salary(request):
    context = {
        'page_title': 'Checking Salary',
    }
    return render(request, 'employee_information/checking_salary.html', context)
...
def visualization(request):
    ...
    return render(request, 'employee_information/visualization.html', context)
...
def create_employee(request):
    if request.method == 'POST':
        ...
        form = EmployeeForm()
    return render(request, 'employee_information/create_employee.html', {'form': form})
...
def delete_employee(request, pk):
    ...
    if request.method == 'POST':
        employee.delete()
        return redirect('home')
    return render(request, 'employee_information/delete_employee.html', {'employee': employee})


@login_required
def result(request):
    print(request)
    MonthlyRate = float(request.POST.get('MonthlyRate'))
    RelationshipSatisfaction = float(
        request.POST.get('RelationshipSatisfaction'))
    StockOptionLevel = int(request.POST.get('StockOptionLevel'))
    YearsSinceLastPromotion = int(request.POST.get('YearsSinceLastPromotion'))

    model = pd.read_pickle('./models/model_web.pickle')
    result = model.predict([[MonthlyRate, RelationshipSatisfaction, StockOptionLevel,
                             YearsSinceLastPromotion]])

    return render(request, 'employee_information/result.html', {'result': result})
